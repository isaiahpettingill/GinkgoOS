...
impl Header {
    pub fn new(size: u64, uuid: [u8; 16]) -> Header {
    ...
    fn create_unencrypted_hash(&self) -> [u8; 16] {
        let mut encoded = [0; 16];
        encoded[..8].copy_from_slice(&self.hash.to_le_bytes());
        encoded
    }

    pub fn encrypted(&self) -> bool {
        self.encrypted_hash != self.create_unencrypted_hash()
    }
...
fn header_valid_test() {
    assert_eq!(Header::new(0, [0; 16]).valid(), true);
}
