#!/usr/bin/env python3
"""Strict bounded QEMU harness for the AHCI NCQ smoke workload."""
...
PASS_PATTERN = re.compile(
    r"^ahci-smoke: PASS "
    r"ncq=([0-9]+) depth=([0-9]+) msi=([0-9]+) interrupts=([0-9]+) "
    r"queue_hwm=([0-9]+) driver_hwm=([0-9]+) prdt_hwm=([0-9]+) "
    r"bytes=([0-9]+) errors=([0-9]+) quarantined=([0-9]+) "
    r"live=([0-9]+) queued=([0-9]+) in_flight=([0-9]+)\r?$",
    re.MULTILINE,
)
FATAL_PATTERNS = (
    ("CPU exception", re.compile(r"\bCPU exception\b", re.IGNORECASE)),
    ("fatal error", re.compile(r"\bfatal\b", re.IGNORECASE)),
    ("smoke failure", re.compile(r"\bahci-smoke:\s*FAIL\b", re.IGNORECASE)),
...
def parse_pass(log: str) -> dict[str, int]:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"malformed AHCI PASS marker\n{log[-10000:]}")
...
def require_metric_limits(metrics: dict[str, int], log: str) -> None:
    checks = (
        (metrics["ncq"] == 1, "ncq must equal 1"),
        (metrics["depth"] > 1, "depth must be greater than 1"),
        (metrics["msi"] == 1, "msi must equal 1"),
        ...
        (metrics["prdt_hwm"] > 1, "prdt_hwm must be greater than 1"),
        (metrics["bytes"] >= 524_288, "bytes must be at least 524,288"),
        (metrics["errors"] == 0, "errors must equal 0"),
        (metrics["quarantined"] == 0, "quarantined must equal 0"),
    ...
    for passed, message in checks:
        if not passed:
            raise RuntimeError(f"AHCI metric check failed: {message}\n{log[-10000:]}")
...
def qemu_command(args: argparse.Namespace, serial_log: str) -> list[str]:
    return [
        ...
        "-drive", f"if=none,id=ginkgo-ahci-disk,format=raw,cache=writethrough,file={args.disk}",
        "-device", "ide-hd,drive=ginkgo-ahci-disk,bus=ginkgo-ahci.0",
...
def run(args: argparse.Namespace) -> dict[str, int]:
    with tempfile.TemporaryDirectory(prefix="ginkgo-ahci-ncq-") as temporary:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if pass_count > 1:
                    raise RuntimeError(f"duplicate AHCI PASS marker\n{log[-10000:]}")
                if pass_count == 1:
                    time.sleep(0.5)
