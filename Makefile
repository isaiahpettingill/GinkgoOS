...
thread-smoke: userspace $(LIMINE_DIR)/BOOTX64.EFI $(OVMF_CODE) $(FS_IMAGE)
	$(THREAD_SMOKE_KERNEL_BUILD)
	rm -rf $(THREAD_SMOKE_ROOT)
	mkdir -p $(THREAD_SMOKE_ROOT)/boot/limine $(THREAD_SMOKE_ROOT)/EFI/BOOT
	cp $(KERNEL) $(THREAD_SMOKE_ROOT)/boot/kernel
	cp limine.conf $(THREAD_SMOKE_ROOT)/boot/limine/limine.conf
	cp $(LIMINE_DIR)/BOOTX64.EFI $(THREAD_SMOKE_ROOT)/EFI/BOOT/
	$(PYTHON) tools/qemu_marker_test.py --qemu "$(QEMU)" --ovmf $(OVMF_CODE) --disk $(FS_IMAGE) --boot-root $(THREAD_SMOKE_ROOT) --success "ginkgo-thread-smoke: PASS"

scheduler-smoke: userspace $(LIMINE_DIR)/BOOTX64.EFI $(OVMF_CODE)
	$(THREAD_SMOKE_KERNEL_BUILD)
	rm -rf $(SCHEDULER_SMOKE_ROOT)
	mkdir -p $(SCHEDULER_SMOKE_ROOT)/boot/limine $(SCHEDULER_SMOKE_ROOT)/EFI/BOOT
	cp $(KERNEL) $(SCHEDULER_SMOKE_ROOT)/boot/kernel
	cp limine.conf $(SCHEDULER_SMOKE_ROOT)/boot/limine/limine.conf
...
request-smoke: userspace $(LIMINE_DIR)/BOOTX64.EFI $(OVMF_CODE)
	...
	rm -rf $(REQUEST_SMOKE_ROOT)
	mkdir -p $(REQUEST_SMOKE_ROOT)/boot/limine $(REQUEST_SMOKE_ROOT)/EFI/BOOT
	cp $(KERNEL) $(REQUEST_SMOKE_ROOT)/boot/kernel
	cp limine.conf $(REQUEST_SMOKE_ROOT)/boot/limine/limine.conf
	cp $(LIMINE_DIR)/BOOTX64.EFI $(REQUEST_SMOKE_ROOT)/EFI/BOOT/
	rm -f $(REQUEST_SMOKE_DISK)
