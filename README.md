...
- Wait registration and direct wakeup are single-core; future SMP needs cross-CPU wake delivery and queue ownership.
...
See [`docs/memory.md`](docs/memory.md) for the memory architecture, VM ABI, accounting policy, failure rules, and test matrix. See [`docs/threads.md`](docs/threads.md) for thread ownership, lifecycle, ABI, fault policy, and SMP assumptions. See [`docs/scheduling.md`](docs/scheduling.md) for scheduling classes, budgets, fairness, donation bounds, and diagnostics. See [`docs/requests.md`](docs/requests.md) for asynchronous request state, buffers, limits, cancellation, drain, driver rules, and diagnostics. See [`docs/storage.md`](docs/storage.md) for the asynchronous block queue, driver contracts, filesystem fiber, cache, durability, recovery, and storage tests. See [`SECURITY.md`](SECURITY.md) for the threat model, trust/update policy, CPU-feature audit, and unsupported hardware assumptions.
...
The production `ginkgo-desktop-service` runs `ginkgo-desktop` policy in ring 3 and drives channel requests, protected surface allocation, placements, focus, and buffer lifecycle through the kernel broker. The integrated path provides server decorations with a clickable close control, pointer focus, focused keyboard/pointer input routing, fullscreen with layout restoration, scrolling columns, and clipping. Decorations are included in the first published frame even when the client reports partial damage. The compositor builds each frame in a RAM backbuffer, then publishes damaged regions through packed framebuffer writes before completing the presentation and advancing `BufferReleased` ownership.

`ginkgo-fonts` provides sorted packed one-bit bitmap glyphs, kerning, allocation-free rendering through `embedded-graphics`, and strict parsing of a versioned little-endian `.gkf` representation. The host-only `yaff2gkf` tool parses YAFF through the pinned `libyaff` fork, normalizes Unicode glyphs and kerning, validates the result, and writes an atomically published `.gkf` file. Applications can embed validated `.gkf` bytes or construct normalized fonts from converter output.

The registry contains the hidden `Ginkgo Desktop` service plus visible `Help`, `Files`, `Text Editor`, `Terminal`, and `Ginkgo Demo` applications. Help explains the integrated keyboard and mouse controls inside GinkgoOS. `Files` lists the persistent user workspace, moves selection with Up/Down, opens entries with Enter, returns with Backspace, and removes files with Delete. Boot stops at an empty desktop until the user launches an app. The demo draws a steady centered “Hello World” surface, and `F11` toggles fullscreen. Normal panes reserve a strip for the system tray while fullscreen remains edge-to-edge. `META+N` toggles the registry-backed launcher, Escape closes it, and its search and app rows use the embedded-icon `Magnify` and `CubeOutline` drawables and bounded background save/restore.

A persistent tray spans the top of the output. It shows uptime, graphical-process CPU use, physical-memory use, network state, and currently open graphical programs. GinkgoOS has no RTC or network stack yet, so the time is labeled `UP` and the network field reports `NET offline`. Clicking GinkgoOS opens the launcher, and the Help button launches the unprivileged Help application.

Currently integrated pane bindings are `META+Left/Right` for focus, `META+Q` to close the focused application, `META+A/S` to move the focused pane left/right, `META+=/-` to adjust its width in 5% steps, and `META+L/C/R` to align it left/center/right. Columns form a horizontally scrolling workspace, so additional running applications can be off-screen; use the focus bindings to navigate them. Windows can also be closed with the X control in their title bar. The remaining desktop hotkey work is tracked in #5.
...
The kernel uses the RedoxFS 0.9.1 transaction engine and filesystem format, adapted from upstream commit `99bc185bf8ad8bd6f4d2562c424d800c2a3d310b`. Boot prefers PCI-discovered transitional `virtio-blk` and falls back to PCI AHCI/SATA. After bootstrap, both drivers run through a bounded asynchronous block queue with interrupt-driven completion and watchdog fallback. Virtio-blk uses MSI-X and fixed split-virtqueue descriptor blocks. AHCI uses MSI, multiple command slots, NCQ when both HBA and disk support it, and validated multi-entry PRDTs.

Storage discovery validates GPT header and partition-array CRCs and bounds, understands protective and legacy MBRs, and mounts the first usable partition. Media with no partition table remains supported as a whole-disk volume. RedoxFS runs on a dedicated stackful fiber, so a hardware wait suspends filesystem execution without stalling unrelated runnable work. A bounded 4 KiB write-back cache batches small writes; sync and shutdown use ordered durability tickets and device flushes when supported. A blank selected volume is formatted once; later boots reopen it, while trusted embedded programs are refreshed without deleting ordinary files.
...
Run `make memory-policy-smoke` for the sequential 256 MiB, 512 MiB, and 5 GiB QEMU matrix. The opt-in kernel verifies high-first ordinary allocation, low DMA32 preservation, HHDM access, user/shared mappings, real userspace stack growth and guard faults, process-scoped OOM recovery, and exact retirement before each boot emits one PASS marker. Its generated probes and failure injection are compiled only when `GINKGO_MEMORY_POLICY_SMOKE=1`.
