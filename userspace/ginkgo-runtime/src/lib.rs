...
unsafe impl Source for ProcessHeapSource {
    fn acquire<B: Binning>(talc: &mut Talc<Self, B>, layout: Layout) -> Result<(), ()> {
        let requested = layout
            ...
            .ok_or(())?;
        let mapping =
            unsafe { anonymous_map(requested, MapProtection::READ | MapProtection::WRITE) }
                .map_err(|_| ())?;
...
#[global_allocator]
static ALLOCATOR: TalcLock<RawSpinlock, ProcessHeapSource> = TalcLock::new(ProcessHeapSource);
...
/// Defines the same entry shim for a function receiving all six integer ABI arguments.
#[macro_export]
macro_rules! entry6 {
    ($entry:path) => {
        core::arch::global_asm!(
